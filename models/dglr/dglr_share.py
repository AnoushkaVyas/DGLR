"""
Function to compute node embeddings for a particular window. GRU is common for all the regions.
...
Function initialisation: (infeat,outfeat,hiddenfeat, device,alpha,nheads)
...
class DGLR_SHARE(torch.nn.Module):
...
class GRCU(torch.nn.Module):
    def __init__(self,nodes,infeat,outfeat,finaloutfeat,alpha,nheads):
        ...
        self.evolve_features=mat_GRU_cell(self.nodes,self.finaloutfeat)
    ...
    def forward(self,A_list,node_embs_list,concat):
        ...
        for t,Ahat in enumerate(A_list):
            ...
            GAT_features = self.evolve_features(GAT_features,node_embs)
...
class mat_GRU_cell(torch.nn.Module):
    def __init__(self,infeat,outfeat):
        ...
        self.rows = infeat
        self.cols = outfeat
        self.update = mat_GRU_gate(self.rows,self.cols,torch.nn.Sigmoid())

        self.reset = mat_GRU_gate(self.rows,self.cols,torch.nn.Sigmoid())

        self.htilda = mat_GRU_gate(self.rows,self.cols,torch.nn.Tanh())
