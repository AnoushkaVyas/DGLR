...
soil moisture which can leverage both spatial and temporal dependency of soil moisture values.
...
from models.dglr.dglr_share import DGLR_SHARE
from models.dglr.dglr import DGLR

#Importing Regression models
from models.regression.Regression_ind import FC_IND
from models.regression.Regression_shared import FC_SHARE
...
parser.add_argument('--model', type=str, default='DGLR', help='Model to be used.')
...
for itr in range(args.noftimes):
    ...
    if args.model=='DGLR_SHARE':
        model_egnn = DGLR_SHARE(nodes=args.nofstations, infeat=features[0].shape[1],outfeat=args.out,hiddenfeat=args.hidden,device="cuda" if args.cuda else "cpu",alpha=args.alpha,nheads=args.nheads)
        model_regression = FC_SHARE(nodes=args.nofstations,ntime=args.window,nfeatures=args.out,negslope=args.negslope,device="cuda" if args.cuda else "cpu")
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(args.nofstations):
            ...
            for wd in range(0,train_set-args.window,args.stride):
                ...
                train_AList=AList[wd:wd+args.window]
                train_data=features[wd:wd+args.window]
            ...
            loss_train=loss_train+loss_wd.item()
    ...
    for i in range(args.nofstations):
        for wd in range(train_set-args.window,train_set+test_set-args.window,args.stride):
            ...
            test_AList=AList[wd:wd+args.window]
